...
constexpr unsigned int NUM_BODIES = (1024 * 2);  // Number of bodies
...
constexpr double SOFTENING = (0.015 * TO_METERS);// Softens body interactions at close distances
constexpr double SOLAR_MASS = 2.0e30;			 // in kg
constexpr double BINARY_SEPARATION = 0.07;		 // AU (only applies when binary code uncommented)
constexpr double EXTRA_MASS = 1.5;				 // 0.02 Disk mask as a portion of center star/black hole massed
constexpr double MAX_DISTANCE = 0.75;			 // 2.0 Barnes-Hut Distance approximation factor
...
constexpr double TIME_STEP = (3 * 32 * 1024);	 // (1*128*1024) Simulated time between integration steps, // in seconds
