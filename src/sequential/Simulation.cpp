...
Simulation::Simulation(unsigned int width, unsigned int height) : _width(width), _height(height),
																   _bodies(Body::generate(NUM_BODIES)),
																   _zoom(1.2f)
{
	// create the window
	_window.create(sf::VideoMode(_width, _height), "N-body simulation", sf::Style::Default);
	// setup the view
...
void Simulation::poll_events()
{
	...
	if (sf::Mouse::getPosition().x > (_width - 20))
		_view.move(2 * _zoom, 0);
	if (sf::Mouse::getPosition().x < (0 + 20))
		_view.move(-2 * _zoom, 0);
	if (sf::Mouse::getPosition().y > (_height - 20))
		_view.move(0, 2 * _zoom);
...
void Simulation::update() {
	...
	for (int i = 0; i < NUM_BODIES; ++i) {
		...
		// update the body after it has interacted all other bodies
		_bodies[i].update(dt); 
	}
	...
	//	body.update(dt); // update the body
	//}
}
...
void Simulation::render()
{
	...
	for (size_t i = 0; i < NUM_BODIES; ++i) {
		...
		// the simulation is essentially in 2D because the z coordinates are not really used
